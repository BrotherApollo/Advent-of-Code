...
for combo in product([0,1], repeat=bot.numButtons):
    print(combo)
    
    lights = [0] * bot.numLights
    presses = sum(combo)
    
    for index, pressed in enumerate(combo):
        if pressed:
            for light in bot.buttons[index]:
                lights[light] ^= 1 #XOR opperator homie
                
    if lights == bot.target:
        bot.min_presses = min(bot.min_presses, presses)

print(bot.min_presses)
